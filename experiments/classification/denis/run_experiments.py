...
def train_test(size, num_classes: int, seed, num_epochs):
    ...
    data['binned'] = pd.qcut(data['Free energy of activation (kcal/mol)'], num_classes, labels=False)
    ...
    classifier = GPTJClassifier("free energy of activation", tune_settings={"num_epochs": num_epochs, "lr": 1e-4}, inference_batch_size=2, inference_max_new_tokens=100)
...
if __name__ == "__main__":
    for num_epochs in [4, 8, 14, 20]:
        for seed in range(5):
            for size in [10,  50, 100, 500, 700][::-1]:
