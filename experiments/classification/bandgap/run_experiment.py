...
def train_test(size, representation, seed, num_epochs):
    df = get_qmug_data()
    df = df.dropna(subset=[representation, 'DFT_HOMO_LUMO_GAP_mean_ev'])
    df['y'] = pd.qcut(df['DFT_HOMO_LUMO_GAP_mean_ev'], 2, labels=[0,1])
    train_df, test_df = train_test_split(df, train_size=size, test_size=min([len(df)-size, MAX_NUM_TEST_POINTS]), random_state=seed)

    classifier = GPTJClassifier("HOMO-LUMO gap", tune_settings={"num_epochs": num_epochs, "lr": 1e-4}, inference_batch_size=2, inference_max_new_tokens=100)
